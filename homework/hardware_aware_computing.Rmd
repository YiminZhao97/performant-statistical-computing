...
v = rep(1, ar_length)
test1 = ar_precision_matvec_R(v, auto_corr)
test2 = ar_precision_matvec(v, auto_corr)
...
t1.end = proc.time()
t1 = t1.end - t1.start
print(paste0('Execution time for R implementation：',round(t1[3][[1]], 6),' seconds'))

#For Rcpp implementation
t2.start = proc.time()
test2 = ar_precision_matvec(v, auto_corr)
t2.end = proc.time()
t2 = t2.end - t2.start
print(paste0('Execution time for Rcpp implementation：',round(t2[3][[1]], 6),' seconds'))
...
matrix.prod.1 = A %*% B
matrix.prod.2 = row_col_dot_matmat(A, B)
sum(matrix.prod.1 == matrix.prod.2)
```
...
```{r}
# Check and benchmark
matrix.prod.3 = col_oriented_matmat(A, B)
sum(matrix.prod.1 == matrix.prod.3)
```
...
t1.end = proc.time()
t1 = t1.end - t1.start
print(paste0('Execution time for %>%：',round(t1[3][[1]], 6),' seconds'))

t2.start = proc.time()
matrix.prod.2 = row_col_dot_matmat(A, B)
t2.end = proc.time()
t2 = t2.end - t2.start
print(paste0('Execution time for row col mat mat：',round(t2[3][[1]], 6),' seconds'))

t3.start = proc.time()
matrix.prod.3 = col_oriented_matmat(A, B)
t3.end = proc.time()
t3 = t3.end - t3.start
print(paste0('Execution time for col mat mat：',round(t3[3][[1]], 6),' seconds'))
```

The reason why Rcpp based method is not better than built in R function may be we use **too many for loops** in the implementation.  
...
```{r}
# Check and benchmark
matrix.prod.4 = matmat_via_eigen(A, B)
sum(matrix.prod.1 == matrix.prod.4)
...
t4.end = proc.time()
t4 = t4.end - t4.start
print(paste0('Execution time for eigen based method：',round(t4[3][[1]], 6),' seconds'))
```

The solution derived by matmat_via_eigen is not totally the same with the other 3 methods.  
